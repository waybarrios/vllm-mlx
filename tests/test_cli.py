...
def test_serve_parser_accepts_memory_budget_gb():
...
def test_serve_parser_rejects_invalid_memory_budget_gb(value, message, capsys):
    from vllm_mlx.cli import create_parser
    ...
    with pytest.raises(SystemExit):
        create_parser().parse_args(
            [
                ...
                "--models-config",
                "/tmp/models.yaml",
                f"--memory-budget-gb={value}",
            ]
        )

    captured = capsys.readouterr()
    assert "--memory-budget-gb" in captured.err
