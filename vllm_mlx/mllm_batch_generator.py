    ...
    def _chunked_next() -> List[MLLMBatchResponse]:
        ...
        if batch_gen._partial is not None:
            ...
            if remaining_count > step:
                ...
                # Eval ALL cache types to break the lazy graph between
                # chunks.  ArraysCache (e.g. GatedDeltaNet) has .state;
                # KVCache (full attention) has .keys/.values.  Hybrid
                # models use both — skipping either lets the graph grow
                # across chunks → OOM on long prompts.
                cache_tensors = []
                for c in partial["cache"]:
        ...
        if batch_gen.unprocessed_requests:
            ...
            if text_only_req is not None:
                ...
                if remaining_count > batch_gen._chunked_prefill_budget:
                    ...
                    # Eval ALL cache types (see partial continuation above)
                    cache_tensors = []
