...
"""Helpers for running MLX operations on a dedicated worker thread.
...
``bind_generation_streams`` is a legacy helper kept for the SimpleEngine path
where a single persistent ``_MLXWorkerThread`` already owns model load +
inference but still needs to rebind module-level stream references.
"""
